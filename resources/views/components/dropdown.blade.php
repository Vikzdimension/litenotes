...
<div class="relative" x-data="{ open: false }" @click.outside="open = false" @close.stop="open = false">
    ...
    <div x-show="open" x-transition:enter="transition ease-out duration-300"
        ...
        class="absolute z-50 mt-2 {{ $width }} rounded-md shadow-lg {{ $alignmentClasses }} transform origin-top transition-all hover:shadow-2xl"
        style="display: none;" @click="open = false">
        <div class="rounded-md ring-1 ring-black ring-opacity-5 {{ $contentClasses }} transition-all">
            {{ $content }}
        </div>
    </div>
</div>
